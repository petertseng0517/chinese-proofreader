        ...
        "errors": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "line": {"type": "integer", "description": "該頁文字的行號（依輸入中標示的行號），從 1 開始"},
                ...
                "required": ["page", "line", "original", "suggestion", "reason"],
...
SYSTEM_PROMPT = (
    "你是專業的繁體中文校稿員。使用者會提供 PDF 逐頁、逐行擷取出的文字"
    "（每行前面以「行號:」標示），請檢查錯別字、錯字、用詞錯誤、明顯的語法錯誤。"
    "只回報你有信心的錯誤，不要回報風格或語氣偏好。"
    "page 與 line 必須完全依照輸入中標示的頁碼與行號回報，不要自行推算或印刷頁碼。"
    "original 欄位必須是該行文字中可被精確逐字比對到的片段（盡量簡短，幾個字即可）。"
...
def extract_pages(pdf_bytes: bytes) -> list[list[str]]:
    """回傳每頁的行清單；頁數採 PDF 實際頁面順序。圖片內容不擷取。"""
...
def build_user_message(pages: list[list[str]]) -> str:
    ...
    for page_num, lines in enumerate(pages, start=1):
        ...
        parts.append(f"=== 第 {page_num} 頁（PDF 實際頁面順序） ===\n{numbered_lines}")
    return "\n\n".join(parts)


def proofread(pages: list[list[str]]) -> list[dict] | None:
    ...
    errors.sort(key=lambda e: (e["page"], e["line"]))
    return errors


def build_report_lines(errors: list[dict]) -> list[str]:
    return [
        f"第{e['page']}頁，第{e['line']}行，「{e['original']}」→ 建議「{e['suggestion']}」"
        for e in errors
    ]
...
def build_excel_report(errors: list[dict], total_chars: int, error_count: int, error_rate: float) -> bytes:
    ...
    headers = ["頁碼（PDF實際順序）", "行號", "錯字", "建議修正", "原因"]
    ws.append(headers)
    for e in errors:
        ws.append([e["page"], e["line"], e["original"], e["suggestion"], e["reason"]])
...
def main():
    ...
    total_chars = sum(len(line) for lines in pages for line in lines)
